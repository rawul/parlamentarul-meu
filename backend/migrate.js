...
(async () => {
    ...
    [... await Deputy.find({}).lean().exec(), ... await Senator.find({}).lean().exec()]
        ...
        .forEach((politician) => {
            User.register({ username: politician.name, email: politician.email, token: '123', politicianId: politician._id }, politician.name === 'Emilut' ? 'test' : generator.generate({ length: 10, numbers: true }))
        });
})()
